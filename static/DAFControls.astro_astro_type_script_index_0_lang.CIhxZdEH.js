...
`;let T=null;async function be(n){if(!T){const e=new Blob([Se],{type:"application/javascript"});T=URL.createObjectURL(e)}await n.audioWorklet.addModule(T)}class Me{_intervalMs;_isActive;_elapsedMs;_getGraph;_getStream;_intervalId=null;constructor(e,t,i,s,l){this._intervalMs=e,this._isActive=t,this._elapsedMs=i,this._getGraph=s,this._getStream=l}start(){this._intervalId||this._isActive()&&(this._intervalId=setInterval(()=>{try{if(!this._shouldSend())return;window.sendAnalyticsEvent?.("daf_active",{event_category:"DAF",event_label:"heartbeat",daf_usage_s:Math.floor(this._elapsedMs()/1e3)})}catch(e){console.warn("Analytics heartbeat error:",e)}},this._intervalMs))}stop(){this._intervalId&&(clearInterval(this._intervalId),this._intervalId=null)}_shouldSend(){if(!this._isActive())return!1;const e=this._getGraph();if(!e||e.state!=="running")return!1;const t=this._getStream();return!t||!t.getAudioTracks?.().some(s=>s.enabled!==!1)||e.gainValue<=0?!1:typeof window.sendAnalyticsEvent=="function"}}const De=200,Fe=`
(function () {
  'use strict';
